...
本分支是面向 TRE 论文场景整理后的模型。详细决策只针对已申报但尚未入场的出口资料箱；进口航次与出口未申报预测余量仅形成箱区—箱型容量预留，不参与具体贝位或排分配。

大计划长表只读取 `new_qty`。它表示尚未入场箱的分配量；`planned_qty` 包含在场箱，因此不作为本模型的需求或预留量。完整边界见 [MODEL_SCOPE.md](MODEL_SCOPE.md)。
