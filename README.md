...
求解采用有限排模式空间上的严格列生成：精确检查所有未激活模式的
Gurobi 约化成本，只有在不存在负约化成本模式后才进入两阶段整数主问题。
该证明阶段不会因整数求解时限而静默提前结束，Gurobi 异常也默认直接报错。

45 英尺箱只能分配到箱区明确识别的边缘大贝位；边缘位置同时禁止 20
英尺箱，且新增 45 英尺与非 45 英尺分配不能混用同一箱区边缘。这一业务
规则保留为硬约束。
...
第二阶段整数主问题的解直接作为最终排级方案输出，不再执行求解后的箱区内部排级重排。
