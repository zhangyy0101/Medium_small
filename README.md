...
| 减少不连续排区数量 | 0.3500 |
| 靠近已有同类箱 | 0.1250 |
| 减少出口排区未利用的保留容量 | 0.2125 |
| 减少按出口分配箱量加权的泊位—箱区距离 | 0.1625 |

箱组跨箱区数量仅作为诊断指标报告，不再进入主目标。每个航次的首个必需箱区和每个箱组的首个必需排区不计入分散惩罚。进口匿名预留不参与这五项目标。峰值利用率不作为额外目标，而是采用数据驱动的 ε 约束：先根据本算例负载与可达剩余容量计算下界，再按可配置余量生成上限；默认余量系数为 0.50。该参数不是码头给定阈值，正式实验必须做敏感性分析。排级精确回填只承担可行性认证与次级质量诊断，不改变主目标、上下界或 gap。
...
未利用排区容量消融使用同一场景和时间预算，并自动按比例重新归一化其余四项目标权重：

```bash
python -m preexperiment --cases pilot_m_201 --paper-time-limit 30 --threads 1 --disable-unused-capacity-objective
```
...
此前数值使用不同目标定义，只保留作开发追溯，不能与当前集成模型的 UB、LB 和 gap 比较。当前目标属于 `integrated_zone_v4`，必须重新运行预实验、消融实验和完整排区 MIP 对照。
