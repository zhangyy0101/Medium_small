...
- `lbbd_selective`：与上述求解器并列的选择性资源状态 LBBD。箱组—贝位数量保持整数，箱组—箱区激活使用候选容量上界收紧的 Big-M 和最少启用箱区覆盖割；profile 状态按资源稀缺性和业务相关性选择整数核心，少量高压力排数作连续松弛，其余排数保持整数。初始化仅临时放松排数以取得候选数量点，随后立即恢复正式主问题类型；联合精确排级 oracle 仅建立当前正数量支持集上的等价稀疏模型，优先把 IIS 中同贝位、同资源类型的小型冲突核提升为无额外二元变量的类型化物理容量割，无法证明更强容量界时才对紧凑 IIS 使用单调割，并对低估的航次排级代价生成局部数量条件最优性割。不可行点另由两段式冲突修复 MIP 先保证首个完整解、再在同一模型中改善，之后以与割支持集解耦的航次均衡排级邻域改善上界。全部阶段共享一个总截止时间，按总时限比例和实时剩余量动态分配，未使用时间自动回流。该入口不调用 M0、`lbbd_profile` 或备用求解链。
