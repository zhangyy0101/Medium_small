...
The implementation has one independent master-oracle-cut loop. Before formal optimization, all row-count variables are relaxed only long enough to obtain a zero-objective quantity probe; their formal variable types are then restored. The persistent joint exact row-recourse MIP is built once and checks that probe. IIS extraction has a separate short time cap; an interrupted extraction still returns an infeasible subsystem and prevents conflict diagnosis from consuming the primal-repair budget. A certified infeasible probe immediately generates a valid cut. A separate restricted exact-row model repairs the point by allowing its active bays plus a small deterministic set of alternatives. For a dense IIS, only the sixteen positive-demand groups with the largest assigned quantities receive the enlarged alternative set; all remaining groups keep the standard small alternative set. The repaired plan is improved by a voyage-balanced row-level neighbourhood: at most twelve conflict or high-loss groups may use all candidate rows while all other groups remain fixed. Batches rank unvisited groups before previously optimized groups and then rank business loss, so successive solves cover different groups instead of repeatedly spending the budget on the same support. Repair and neighbourhood bounds never enter the global proof; only their independently validated incumbents are mapped to the formal master as MIP Starts.
...
Two valid cut classes are separated explicitly. The exact row model is componentwise downward-closed in its fixed nonnegative quantity requirements. For an infeasible IIS core of at most 64 positive quantity keys, a small footprint-packing certificate first maximizes the core's attainable physical capacity. If its certified upper bound is below the incumbent core quantity, the master receives the stronger linear capacity inequality without new binary variables. Larger cores skip this auxiliary MIP. Whenever the certificate cannot prove a stronger bound, the fallback monotone IIS cut requires at least one core quantity to decrease; merely increasing a conflicting quantity no longer escapes the cut. When exact recourse has a certified lower bound above `theta`, a quantity-conditional optimality cut is added. All cuts are conditioned only on the actual first-stage quantity vector. No primal support state is permanently promoted, and no matching loop, per-voyage verification chain, M0 repair, or fallback solver is present.
